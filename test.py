...
def make_model(src_vocab, tgt_vocab, N=6, d_model=512, d_ff=2048, h=8, dropout=0.1):
    "帮助: 从超参数中构建一个模型"
    copy_layer = copy.deepcopy
    attn = MultiHeadedAttention(d_model, h)
    ff = PositionwiseFeedForward(d_model, d_ff, dropout)
    position = PositionalEncoding(d_model, dropout)
    model = EncoderDecoder(
        Encoder(EncoderLayer(d_model, copy_layer(attn), copy_layer(ff), dropout), N),
        Decoder(DecoderLayer(d_model, copy_layer(attn), copy_layer(attn), c(ff), dropout), N),
        nn.Sequential(Embeddings(d_model, src_vocab), copy_layer(position)),
        nn.Sequential(Embeddings(d_model, tgt_vocab), copy_layer(position)),
